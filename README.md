...
#### Optional MTProto Proxy Configuration:
If your network requires routing through an MTProto proxy:
* `TG_MTPROTO_IP`: Proxy IP address or hostname.
* `TG_MTPROTO_PORT`: Proxy port number (default: `443`).
* `TG_MTPROTO_SECRET_PATH`: File path to the proxy key/secret file.
* `TG_MTPROTO_SECRET`: Alternatively, pass the raw key/secret oneline string directly.
